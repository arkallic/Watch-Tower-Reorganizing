    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            processed_stats = {
                ...
                "humanMembers": len(human_users),
                "botMembers": len(users) - len(human_users),
                "onlineMembers": len([u for u in users if u.get('status') == 'online']),
                "guildChannels": guild_info.get("channels", {}).get("total_channels", 0),
                "guildRoles": len(guild_info.get("roles", [])),
                "serverBoosts": guild_info.get("guild", {}).get("premium_subscription_count", 0),
                "serverBoostLevel": guild_info.get("guild", {}).get("premium_tier", 0),
                ...
                "totalFlags": dashboard_stats.get("general", {}).get("ai_flags", 0),
                "messageVelocity": dashboard_stats.get("general", {}).get("messages_per_hour", 0),
                "botOnline": "error" not in bot_status,
                "botLatency": f"{round(bot_status.get('connection', {}).get('latency', 0))}ms",
                "ollamaConnected": bot_status.get("integrations", {}).get("ollama_connected", False),
            }

            top_users = sorted(
                [u for u in human_users if u.get('total_cases', 0) > 0],
                key=lambda u: u.get('risk_score', 0),
                reverse=True
            )[:5]

            return {
                "stats": processed_stats,
                "systemHealth": raw_data.get("systemHealth", {}),
                "topUsers": top_users
            }
...
async def get_users_page_enhanced_data():
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            user_stats = {
                "total": len(users),
                "humans": len(human_users),
                "bots": len(users) - len(human_users),
                "clean": len([u for u in human_users if u.get('total_cases', 0) == 0]),
                "highRisk": len([u for u in human_users if u.get('risk_level') in ['High', 'Critical']]),
                "newMembers": len([u for u in human_users if u.get('server_tenure_days', 999) <= 7]),
            }

            risk_distribution = [
              {"name": 'Low', "value": len([u for u in human_users if u.get('risk_level') == 'Low']), "color": '#22c55e'},
              {"name": 'Medium', "value": len([u for u in human_users if u.get('risk_level') == 'Medium']), "color": '#f59e0b'},
              {"name": 'High', "value": len([u for u in human_users if u.get('risk_level') == 'High']), "color": '#ef4444'},
              {"name": 'Critical', "value": len([u for u in human_users if u.get('risk_level') == 'Critical']), "color": '#a855f7'}
            ]

            server_growth = []
            daily_stats = trends_data.get("daily_stats", [])
            if daily_stats:
                member_count = len(human_users)
                for day in reversed(daily_stats):
                    server_growth.insert(0, {"date": day.get("date"), "members": member_count})
                    member_count -= (day.get("new_members", 0) - day.get("left_members", 0))

            return {
                "users": users,
                "userStats": user_stats,
                "riskDistribution": [item for item in risk_distribution if item["value"] > 0],
        ...
        try:
            ...
            for case in cases:
                if case.get("created_at"):
                    key = datetime.fromisoformat(case["created_at"].replace("Z","")).strftime('%Y-%m-%d')
            ...
            severity_data = [{"name": "Low", "value": severity_counts["Low"], "color": "#22c55e"}, {"name": "Medium", "value": severity_counts["Medium"], "color": "#f59e0b"}, {"name": "High", "value": severity_counts["High"], "color": "#ef4444"}, {"name": "Critical", "value": severity_counts["Critical"], "color": "#a855f7"}]
        ...
        try:
            ...
            if range == "24h":
                for i in range(23, -1, -1):
                    ...
                    labels.append(hour)
                    data_map[hour] = {"messages": 0, "cases": 0, "kicks": 0, "bans": 0, "notes": 0}
            else:
                days = 7 if range == "7d" else 30
                for i in range(days - 1, -1, -1):
                    date = (now - timedelta(days=i)).strftime("%Y-%m-%d")
                    labels.append(date)
                    data_map[date] = {"messages": 0, "cases": 0, "kicks": 0, "bans": 0, "notes": 0}
            for case in all_cases:
                try:
                    ...
                    if key in data_map:
                        data_map[key]["cases"] += 1
                        action = case.get("action_type", "").lower()
...
async def get_channels_page_data():
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            response = await client.get(f"{BOT_API_URL}/stats/channels/activity-summary")
            response.raise_for_status()
